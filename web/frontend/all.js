...
jQuery(function($){
	...
	function onBeforeSubmit(data, $form, options) { 
		    ...
		    $window.html('<div class="loader"><span class="fa fa-spinner fa-pulse"></span></div>');
		    $window.css({
		    	background: 'none'
		    });
		    $window.popup('show');
	...
	function onError(){
		$window.html('<h1>Erreur</h1><p>Désolé, le formulaie n\'a pas été envoyé. Veuillez Vérifier votre connexion internet!</p>' );
		$window.css({
		    	backgroundColor: '#fff'
		    });
		$window.popup('show');
...
jQuery(function($) {
		rotatorWidth=$('body').innerWidth()-300;
		rotatorHeight=$('.rotator__slide').height();
		$('.rotator').width=rotatorWidth;
		$( ".rotator .rotator__content" ).rcarousel( {
			width: rotatorWidth, 
		...
		$('.rotator__content').center({
			against:'.rotator'
		});
		function generatePages() {
					var _total, i, _link;
					
					_total = $( ".rotator__content" ).rcarousel( "getTotalPages" );
					...
					for ( i = 0; i < _total; i++ ) {
						...
						$(_link)
							.bind("click", {page: i},
								function( event ) {
									$( ".rotator__content" ).rcarousel( "goToPage", event.data.page );
									event.preventDefault();
								}
							)
							.addClass( "rotator__bullet is-off" )
							.appendTo( ".rotator__navigation" );
					}
					
					// mark first slide as active
					$( "a:eq(0)", ".rotator__navigation" )
						.removeClass( "is-off" )
						.addClass( "is-on" )
				...
				function pageLoaded( event, data ) {
					$( ".rotator__bullet.is-on", ".rotator__navigation" )
						.removeClass( "is-on" )
						.addClass('is-off')

					$( ".rotator__bullet", ".rotator__navigation" )
						.eq( data.page )
						.addClass( "is-on" )
						.removeClass( "is-off" )
				}				
...
jQuery(function($){
     $( ".roller .roller__content" ).rcarousel( {
			width: $('body').innerWidth(), 
			height: $('.roller .roller__slide').height(),
			auto: {
				enabled: true,
				interval: 1500
			},
