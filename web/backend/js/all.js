...
jQuery(function($){
	//card
	$('.card .card__front').addClass('front');
	$('.card .card__back').addClass('back');
	$('.card').flip({
		trigger: 'hover'
	});
	//gallery
	var $gallery=$('.gallery .gallery__body').imagesLoaded(function(){
		$gallery
			.isotope({
				itemSelector: '.gallery__cell',
				layoutMode: 'fitRows'
				
			})
			.isotopeSearchFilter({
				  itemsContainer: $gallery,
			      itemSelector: '.gallery__cell',
			      filtersSelector: '.filters',
			      searchResultsClassSelector: '.active',
			      inputSearch: $('#gallery__search')
			});
	});
	//multiselect
	$('select').multiselect({
		nonSelectedText: 'Sélectionner une option',
        nSelectedText: 'sélectionné(e)',
        allSelectedText: 'Tous/toutes'
	});
});